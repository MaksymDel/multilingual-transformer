...
class TrainMultilingual(Subcommand):
...
def datasets_from_params(params: Params) -> Dict[str, Iterable[Instance]]:
    ...
    logger.info("Reading training data from following files: %s", ", ".join(train_data_paths))
    logger.info("These files correspond to the following language pairs: %s", ", ".join(language_pairs))
    if len(language_pairs) != len(train_data_paths):
        raise ConfigurationError("Number of language pairs is not equal to the number of data files provided")

    train_datasets = []
    for i, train_data_path in enumerate(train_data_paths):
        train_datasets.append(dataset_readers[i].read(train_data_path))

    datasets: Dict[str, Iterable[Instance]] = {"train": train_datasets}

    # ================ Validation data ===================
    validation_data_paths = params.pop('validation_data_paths', None)
    if validation_data_paths is not None:
        validation_data_paths = validation_data_paths.split(", ")
        logger.info("Reading validation data from %s", validation_data_paths)

        validation_datasets = []
        for i, validation_data_path in enumerate(validation_data_paths):
            validation_datasets.append(validation_and_test_dataset_readers[i].read(validation_data_path))

        datasets["validation"] = validation_datasets

    test_data_path = params.pop("test_data_paths", None)
    ...
    return datasets, language_pairs
...
def train_model(params: Params,
    ...
    all_datasets, language_pairs = datasets_from_params(params)
    ...
    vocab = Vocabulary.from_params(
            params.pop("vocabulary", {}), (instance for dataset in all_datasets["train"] for instance in dataset)
    )
