...
@Model.register("rnn_search")
class RnnSearch(Model):
    """
    This is ``SimpleSeq2Seq`` model with some minor advances and additional training parameters.

    It which takes a sequence, encodes it, and then
    ...
    def __init__(self,
                 ...
                 label_smoothing_ratio: float = None) -> None:
        super(RnnSearch, self).__init__(vocab)
    ...
    def forward(self,  # type: ignore
        ...
        if not self._beam_search or target_tokens:
    ...
    def _forward_loop(self,
        ...
        if target_tokens:
            ...
            loss = self._get_loss(logits, targets, target_mask, label_smoothing_ratio=self._label_smoothing_ratio)
            output_dict["loss"] = loss
            self.bleu(all_predictions, targets)
    ...
    def _get_loss(logits: torch.LongTensor,
                  ...
                  label_smoothing_ratio: float = 0.0) -> torch.Tensor:
        """
        Compute loss.
